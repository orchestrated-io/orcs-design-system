import Avatar from ".";
import Box from "../Box";
import StyledLink from "../StyledLink";
import { Meta, Story, Preview, Props } from "@storybook/addon-docs/blocks";

<Meta title="Components/Avatar" component={Avatar} />

# Avatar

[Properties](#properties) | [Examples](#examples) | [Upgrading](#upgrading)

Avatars can come in a few different options. Firstly regular or small size (using a H2 or H5 respectively).

Next, depending on what is available it can preferably contain an image file/source, if no image, then show initials as fall back, if no name/initials, then fall back to an icon.

Thirdly the title/name can be a link or just plain text.

Lastly you can specify whether it is being rendered on a dark background and the colors will change to suit this case.

Also, if no title/subtitle is specified then it will only show the circle avatar part.

Follow example code below to achieve desired option.

<Preview>
  <Story name="Basic">
      <Avatar title="Ayden Lundgre" subtitle="Senior Business Analyst" />
  </Story>
</Preview>

## Properties

You can use `space` and `layout` Styled-System properties with `Avatar`, as well as two custom props: `size` and `color`.

### For reference, see:

- [space on Styled-System](https://styled-system.com/table#space)
- [layout on Styled-System](https://styled-system.com/table#layout)
- [systemtheme.js](https://github.com/orchestrated-io/orca-design-system/blob/master/packages/orcs-design-system/lib/systemtheme.js) (our arrays for design system values)

By default, we've set certain values on `Avatar` properties as documented in the table below. If these properties are not specified they will take on the default values.

<Props of={Avatar} />

## Examples

<Preview>
  <Story name="With Initials">
  <Avatar
    title="Ayden Lundgre"
    subtitle="Senior Business Analyst"
    initials="AL"
  />
  </Story>
    <Story name="With Image">
  <Avatar
    title="Ayden Lundgre"
    subtitle="Senior Business Analyst"
    initials="AL"
    image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
  />
  </Story>
  </Preview>

  <Preview>
    <Story name="With Name as Link">
  <Avatar
    title={<StyledLink href="#">Ayden Lundgre</StyledLink>}
    subtitle="Senior Business Analyst"
    initials="AL"
    image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
  />
  </Story>
  </Preview>

## Size
  <Preview>
    <Story name="Small">
    <>
    <Box>
    <Avatar size="small" title="Ayden Lundgre" subtitle="Senior Business Analyst" />
    </Box>
    <Box>
    <Avatar
      size="small"
      title="Ayden Lundgre"
      subtitle="Senior Business Analyst"
      initials="AL"
    />
    </Box>
    <Box>
    <Avatar
      size="small"
      title="Ayden Lundgre"
      subtitle="Senior Business Analyst"
      initials="AL"
      image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
    />
    </Box>
    <Box>
    <Avatar
      size="small"
      title={<StyledLink href="#">Ayden Lundgre</StyledLink>}
      subtitle="Senior Business Analyst"
      initials="AL"
      image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
    />
    </Box>
    </>
    </Story>
</Preview>

<Preview>
<Story name="Inverted">
<>
    <Box bg="greyDarkest">
    <Avatar
      color="white"
      title="Ayden Lundgre"
      subtitle="Senior Business Analyst"
      initials="AL"
      image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
    />
    </Box>
    <Box bg="greyDarkest">
    <Avatar
      color="white"
      title="Ayden Lundgre"
      subtitle="Senior Business Analyst"
      initials="AL"
      />
    </Box>
    <Box bg="greyDarkest">
    <Avatar
      color="white"
      title={<StyledLink href="#">Ayden Lundgre</StyledLink>}
      subtitle="Senior Business Analyst"
      initials="AL"
      image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
    />
    </Box>
    <Box bg="greyDarkest">
    <Avatar
      color="white"
      size="small"
      title="Ayden Lundgre"
      subtitle="Senior Business Analyst"
      initials="AL"
      image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
    />
    </Box>
    <Box bg="greyDarkest">
    <Avatar
      color="white"
      size="small"
      title={<StyledLink href="#">Ayden Lundgre</StyledLink>}
      subtitle="Senior Business Analyst"
      initials="AL"
      image="https://s3.amazonaws.com/uifaces/faces/twitter/jsa/128.jpg"
    />
    </Box>
    </>
  </Story>
  </Preview>
