...
const expandedNavItemStyles = (props) => styledCss`
  ...
  .nav-item-text {
    ...
    font-weight: ${themeGet("fontWeights.1")};
    color: ${
      props.active ? themeGet("colors.white") : themeGet("colors.greyDarker")
    };
    white-space: nowrap;
    overflow: hidden;
  ...
  &:focus .nav-item-text {
    color: ${
      props.active ? themeGet("colors.white") : themeGet("colors.primary")
    };
  }
`;
      ...
      {item.actionType === "link" ? (
        ...
        >
          <Component item={item}>
            ...
            {isExpanded && <span className="nav-item-text">{item.name}</span>}
          </Component>
        </SideNavItemLink>
      ) : (
        <SideNavItem
