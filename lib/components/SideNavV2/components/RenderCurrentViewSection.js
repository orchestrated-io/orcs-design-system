...
const RenderCurrentViewSection = ({ viewingState }) => {
  ...
  // Don't render if there's no valid viewing state, no name, or no container
  if (!viewingState || !viewingState.name || !container) {
    return null;
  }

  return createPortal(
    <Flex flexDirection="column" gap="s" width="100%">
