...
import useRandomUUID from "../../hooks/randomUUID";
...
const TooltipControl = styled.button`
  border: none;
  background: none;
  padding: 0;
  cursor: help;
...
export default function Popover({
  ...
  const [isActive, tooltip] = useIsVisible();
  const ariaDescribedBy = useRandomUUID("popover");
  ...
  const component = (
    ...
    >
      ...
      {!!text && (
        ...
        >
          <span id={ariaDescribedBy} role="listitem" ref={tooltip}>
            {text}
          </span>
        </Text>
      )}
