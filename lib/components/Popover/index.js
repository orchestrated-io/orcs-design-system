...
export default function Popover({
  ...
  const offsetFunction = useCallback(
    ({ rects }) => {
      const defaultOffset = {
        mainAxis: 10,
  ...
  const style = useMemo(() => {
    ...
    if (headerAvatarSizing === "large") {
      baseStyle.transform = `${baseStyle.transform || ""} translateY(-56px)`;
    } else if (headerAvatarSizing) {
      baseStyle.transform = `${baseStyle.transform || ""} translateY(-3px)`;
    }
