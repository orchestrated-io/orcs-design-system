...
const HeaderAvatarActionPopover = styled(Popover)(({ $shape }) => ({
  position: "absolute",
  top: 0,
  right: 0,
...
const HeaderAvatarAction = styled("button")(({ ...props }) => ({
  width: "36px",
  height: "36px",
  borderRadius: "50%",
  background: "#e6e6e6",
  ...
  if (headerAvatar) {
    ...
    return (
      <HeaderAvatarWrapper {...props} $size={size} $shape={shape}>
        ...
        {onClick && sizing === "large" && (
          <HeaderAvatarActionPopover
            direction="right"
            width="fit-content"
            $shape={shape}
            text={
              shape === "circle" ? "Upload profile picture" : "Change image"
            }
          >
            <HeaderAvatarAction onClick={onClick} aria-label="Edit avatar">
              {shape === "circle" ? (
                <Icon icon={["fas", "camera"]} size="lg" color="greyDarker" />
              ) : (
                <Icon
                  icon={["fas", "edit"]}
                  size="lg"
