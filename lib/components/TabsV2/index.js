...
const TabsV2 = ({ tabsList }) => {
  ...
  const showMoreVisible = actionsMoreList.length > 1;
  const location = useLocation();
  const showMoreItemActive = actionsMoreList.find((action) =>
      ...
      <TabWrapper>
        <Tabs style={{ gap: tabsGap }} ref={containerRef}>
          {tabsList.map((action) => (
            <Tab
              key={action.path}
              tabHidden={showMoreVisible ? action.hideTab : false}
              to={action.path}
            >
              {action.label}
            </Tab>
            ...
            customTriggerComponent={
              ...
              >
                ...
                <FlexItem flex="0 0 auto">
                  <Icon icon={["fas", "chevron-down"]} title="Down" ml="s" />
                </FlexItem>
              </ShowMoreButton>
            }
            closeOnClick
