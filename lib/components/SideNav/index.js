...
const SideNav = ({ items, sideNavHeight }) => {
  ...
  const currentActiveItemIndex = items.findIndex((item) => item.isActive);
  const previousActiveItemIndex = usePrevious(currentActiveItemIndex);
  const previousExpandedItemIndex = usePrevious(expandedItem);

  useEffect(() => {
    const itemToExpand = items.findIndex(
      (item) => item.isExpandedByDefault && !item.hide
    );

    const activeIsItemToExpand = currentActiveItemIndex === itemToExpand;

    if (
      currentActiveItemIndex >= 0
        ? currentActiveItemIndex === previousActiveItemIndex &&
          activeIsItemToExpand
        : itemToExpand === previousExpandedItemIndex
    ) {
      return;
    }

    if (itemToExpand >= 0 && !expandedItem) {
      setExpandedItem(itemToExpand);
    } else if (items[expandedItem]?.hide) {
