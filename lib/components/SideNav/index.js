  ...
  return item.actionType === "link" ? (
    ...
    >
      <SideNavItem
        ...
        active={item.index === activeItem}
        onClick={() =>
          handleItemClick(item.index, item.actionType, item.onClick)
        }
        bottomAligned={item.bottomAligned}
        {...accessibilityProps}
      >
...
const SideNav = ({ items, height }) => {
  const [expandedItem, setExpandedItem] = useState(null);
  const [activeItem, setActiveItem] = useState(null);

  const handleItemClick = (itemIndex, actionType, onButtonClick) => {
        ...
        >
          ...
          >
            <PanelControl
              onClick={() =>
                handleItemClick(
                  allItems[expandedItem].index,
                  allItems[expandedItem].actionType,
                  allItems[expandedItem].onClick
                )
              }
...
SideNav.propTypes = {
  height: PropTypes.string.isRequired, // Used to specify the height of the sideNav
