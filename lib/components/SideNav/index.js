...
const SideNav = ({ items, sideNavHeight }) => {
  ...
  // Initialize a ref for SideNavExpanded and navItem
  const expandedRef = useRef(null);
  // const navItemRef = useRef(null);
  const [navItemRefs, setNavItemRefs] = useState([]);
  ...
  const handleItemClick = (item) => {
    ...
    if (actionType === "link" || actionType === "button") {
      ...
      navItemRefs[itemIndex] &&
        navItemRefs[itemIndex].current &&
        navItemRefs[itemIndex].current.focus();
  ...
  useEffect(() => {
    // Add or remove refs as needed
    setNavItemRefs((navItemRefs) =>
      Array(allItems.length)
        .fill()
        .map((_, i) => navItemRefs[i] || createRef())
    );
  }, [allItems.length]);
