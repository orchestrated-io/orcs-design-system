...
const MultiValueRemove = (props) => {
  ...
  return (
    <components.MultiValueRemove
      {...{
        ...props,
        innerProps: { ...innerProps, "aria-label": `Remove ${data.label}` }
      }}
    />
  );
};
...
const Select = forwardRef((props, ref) => {
  ...
  const customStyles = updateStyles({
    ...
    control: (provided, state) => ({
      ...
      minHeight: props.height
        ? props.height
        : themeGet("appScale.inputHeightDefault")({ theme }),
      boxShadow: "none",
      opacity: state.isDisabled ? 0.7 : 1,
      "&:hover": {
        borderColor: props.invalid
          ? themeGet("colors.danger")({ theme })
          : themeGet("colors.primary")({ theme })
      },
      "&:focus-within": {
        outline: "0",
        ...
        boxShadow: props.invalid
          ? themeGet("shadows.thickOutline")({ theme }) +
            " " +
            themeGet("colors.danger30")({ theme })
          : themeGet("shadows.thickOutline")({ theme }) +
            " " +
            themeGet("colors.primary30")({ theme })
      },
  ...
  const components = { MultiValueRemove, ...(props?.components || {}) };

  const SelectComponent =
    SELECT_TYPES[props?.selectType] ?? SELECT_TYPES.default;
