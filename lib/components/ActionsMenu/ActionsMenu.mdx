...
The child components `ActionsMenuItem` are rendered either as a react router link `<Link>`, standard html hyperlink `<a>`, or button `<button>`. Depending on what the function of the menu item needs to be, just use `to`, `href`, or `onClick={function} + type="button"` and this component will render the correct element.
