...
const castDirectionToPlacement = (direction) => {
  switch (direction) {
    case "left":
      return "left-start";
    case "right":
...
export const ActionsMenuBody = ({
  ...
  const actionMenu = useActionMenu({
    placement: placement || castDirectionToPlacement(direction),
    open: toggleState,
    onOpenChange: (_, e) => {
      if (e) {
        onToggle?.(e);
  ...
  if (customTriggerComponent) {
    triggerComponent = React.cloneElement(customTriggerComponent, triggerProps);
  }
